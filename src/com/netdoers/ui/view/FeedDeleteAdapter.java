...
public class FeedDeleteAdapter extends BaseAdapter {
	...
	public FeedDeleteAdapter(Context mContext, String TAG, ArrayList<Feed> mFeed,
...
//				mContext.startActivity(mCommentIntent);
	...
	private void setPopUpWindow(final Context mContext, View mView, final ArrayList<Feed> mFeed, final Feed mFeedObj,final int position){
		...
		mPopUpMenu.getMenuInflater().inflate(R.menu.menu_feed_pop_up_delete, mPopUpMenu.getMenu());	
		mPopUpMenu.setOnMenuItemClickListener(new PopupMenu.OnMenuItemClickListener() {
			...
			public boolean onMenuItemClick(MenuItem item) {
				...
				case R.id.menu_feed_delete:
					volleyDelete(mFeed, position, mFeedObj, "1");
					break;
				}
				return false;
			}
...
//		ImageView mFeedTrue, mFeedFalse;
	...
	private void volleyDelete(final ArrayList<Feed> mFeed, final int position,
			final Feed mFeedObj, final String isReport) {
		final String TAG = "volleyDelete";
		NetworkVolley nVolley = new NetworkVolley();
		JSONObject mJSONObject = RequestBuilder.getPostReportDeleteFeedData(mFeedObj
				.getFeedId());
		Log.i(TAG, AppConstants.URLS.URL_POST_FEED_REPORT_DEL);
		Log.i(TAG, mJSONObject.toString());
		VolleyPostJsonRequest req = nVolley.new VolleyPostJsonRequest(AppConstants.URLS.URL_POST_FEED_REPORT_DEL, mJSONObject,
				new Listener<JSONObject>() {
					@Override
					public void onResponse(JSONObject response) {
						try {
							...
							Log.i(TAG, response.toString());
							if (response.getBoolean("success")) {
								FeedDeleteAdapter.this.mFeed.remove(position);
